...
FNG_FEAR_THRESHOLD = 25    # ниже этого - "Extreme Fear", разрешаем более смелый вход
RSI_LOW_FEAR = 25          # в зоне Extreme Fear допускаем вход при более низком RSI (не только 40-70)
...
def run_backtest(df_entry: pd.DataFrame, df_trend: pd.DataFrame, capital: float = 100.0,
    ...
    """
    fng_mode:
      "none"       - без сентимент-фильтра (как раньше)
      "greed"      - блокируем вход, если FNG > FNG_GREED_THRESHOLD (Extreme Greed)
      "greed_fear" - то же + доп. разрешение на вход при FNG < FNG_FEAR_THRESHOLD
                     (Extreme Fear) даже если RSI ниже обычного порога RSI_LOW,
                     но не ниже RSI_LOW_FEAR - ловим просадки на панике.
    """
    ...
    for ts, row in df.iterrows():
        ...
        if position is None:
            ...
            # базовое условие входа (как в стратегии без сентимента)
            entry_ok = uptrend and cross_up and (RSI_LOW <= row["rsi"] <= RSI_HIGH)

            if fng_mode == "greed_fear" and not entry_ok and pd.notna(fng_value):
                # доп. лазейка: паника на рынке + тренд вверх + RSI хотя бы не в свободном падении
                if fng_value < FNG_FEAR_THRESHOLD and uptrend and cross_up:
                    if RSI_LOW_FEAR <= row["rsi"] <= RSI_HIGH:
                        entry_ok = True

            if fng_mode in ("greed", "greed_fear") and entry_ok and pd.notna(fng_value):
                if fng_value > FNG_GREED_THRESHOLD:
...
def main():
    ...
    for days in TEST_PERIODS_DAYS:
        ...
        fng_min = df_entry_period["fng"].min()
        fng_max = df_entry_period["fng"].max()
        fng_mean = df_entry_period["fng"].mean()

        print(f"--- Период: последние {days} дней ({ENTRY_TF}) ---")
        print(f"  Fear&Greed за период: min={fng_min:.0f}  max={fng_max:.0f}  среднее={fng_mean:.0f}")

        print("  Без сентимент-фильтра:      ", end="")
        ...
        print("  Только Extreme Greed блок:  ", end="")
        trades_greed, equity_greed = run_backtest(df_entry_period, df_trend_period, capital=100.0,
                                                   fng_mode="greed")
        summarize(trades_greed, 100.0, equity_greed, df_entry_period)

        print("  Greed блок + Fear доп.вход: ", end="")
        trades_gf, equity_gf = run_backtest(df_entry_period, df_trend_period, capital=100.0,
                                             fng_mode="greed_fear")
        summarize(trades_gf, 100.0, equity_gf, df_entry_period)
        print()

    print("=" * 60)
    print("Как читать результаты:")
    print(f"- min/max/среднее FNG показывает, доходил ли индекс вообще до порогов")
    print(f"  Extreme Greed (>{FNG_GREED_THRESHOLD}) или Extreme Fear (<{FNG_FEAR_THRESHOLD}) на этом периоде.")
    print("  Если max < 75 - блок по жадности физически не мог сработать (как и было видно раньше).")
    print("  Если min > 25 - доп.вход по панике тоже не мог сработать.")
    print("- Сравнивай 3 строки на каждом периоде: если 'Только Extreme Greed блок'")
    print("  не отличается от базовой - жадности не было. Если 'Greed+Fear' даёт")
    print("  больше сделок и PF выше - паника на этом активе была точкой входа.")
    print("- Если пороги (75/25) редко достигаются - попробуй смягчить их (65/35),")
    print("  индекс не всегда доходит до крайних значений на коротких периодах.")
