"""
Pairs Trading (Relative Value) — ETH/BTC ротация без шорта + проверка "резинки"
=================================================================================
...
ВАЖНО — перед бэктестом скрипт теперь ПРОВЕРЯЕТ саму гипотезу: действительно ли
соотношение ETH/BTC тяготеет к среднему ("резинка есть"), или оно просто уходит
в одну сторону без возврата ("резинки нет"). Это тест Дики-Фуллера (ADF) на
стационарность логарифмического спреда. Если тест говорит "резинки нет" —
бэктест ниже можно не воспринимать всерьёз, каким бы красивым он ни казался.

Требования: pip install pandas numpy requests statsmodels
...
def check_rubber_band(log_ratio: pd.Series):
    """
    Тест Дики-Фуллера (ADF) на стационарность соотношения ETH/BTC.
    Простыми словами: проверяет, есть ли "резинка", которая тянет соотношение
    обратно к среднему, или оно свободно уходит в одну сторону без возврата.

    Возвращает (is_rubber_band: bool, p_value: float, verdict: str)
    """
    clean = log_ratio.dropna()
    if len(clean) < 30:
        return None, None, "Недостаточно данных для теста"

    result = adfuller(clean)
    p_value = result[1]

    # p_value < 0.05 - статистически значимо отвергаем гипотезу "резинки нет"
    # (стандартный порог значимости в статистике)
    is_rubber_band = p_value < 0.05

    if is_rubber_band:
        verdict = "РЕЗИНКА ЕСТЬ - соотношение статистически тяготеет к среднему"
    else:
        verdict = "РЕЗИНКИ НЕТ - соотношение может уходить в одну сторону без возврата"

    return is_rubber_band, p_value, verdict
...
def main():
    ...
    for days in TEST_PERIODS_DAYS:
        ...
        is_rubber_band, p_value, verdict = check_rubber_band(df_period["log_ratio"])
        if p_value is not None:
            print(f"  Проверка резинки (ADF-тест): p-value={p_value:.4f} -> {verdict}")
        else:
            print(f"  Проверка резинки: {verdict}")
        ...
        if p_value is not None and not is_rubber_band:
            print("  ⚠ Внимание: резинки не обнаружено - результат бэктеста выше")
            print("    статистически ненадёжен, даже если цифры выглядят неплохо.")
        print()

    print("=" * 70)
    print("Как читать результаты:")
    print("- 'Проверка резинки' - самое важное число. Если p-value < 0.05,")
    print("  значит статистика подтверждает: соотношение ETH/BTC реально тяготеет")
    print("  к среднему, и стратегию имеет смысл рассматривать всерьёз.")
    print("  Если p-value >= 0.05 - 'резинки' нет, соотношение может уходить")
    print("  в одну сторону без возврата, и красивые цифры бэктеста ниже - совпадение.")
    ...
    print("  если стратегия обгоняет ОБА бенчмарка И резинка подтверждена -")
    print("  относительная стоимость даёт реальный edge.")
